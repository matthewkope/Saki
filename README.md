...
The verison of the cue algorithm that was reverse engineered was during the months of Jan - Mar 2026. It has since been updated, what was once my highest 91% compatiability is now a 85% compatiability. For the lower tiers what once was a 35% is now around 50%. What is interesting is that the calculations still work on the locations or clothing portions more accuratly as I don't think they were updated yet. 

The fb-compatibility-extension allows the user to unlock a secret page on the website. It scapes all your friends birthdays and runs a compatibility test on them.
...
The Lucky Number is derived from the first and last meaningful digits of the birth date, with three fallback steps.

### Lucky Number Formula

1. Take the **first digit of the birth month** (e.g. month 11 → first digit = 1)
2. Take the **last non-zero digit of the birth year** (e.g. year 2000 → last non-zero digit = 2, not 0)
3. Concatenate them (e.g. 2 and 2 → 22)

Then apply the following priority order:

- **Step 1** — If the combined number is **not 19** → that is the Lucky Number (19 is the only invalid result)
- **Step 2** — If combined **= 19** → use the **birth day** instead. If the birth day is not 19 → that is the Lucky Number
- **Step 3** — If the birth day is **also 19** → sum all digits of the birth year and reduce (stopping at 11, 22, 28, 33) → that is the Lucky Number

### Examples

**Example 1 — Birth day is the lucky number:** 11/09/1999

```text
First digit of month (11) = 1
Last non-zero digit of year (1999) = 9
Combined: 19  →  > 9, not master → go to Step 2
Birth day = 9  →  ≤ 9 → Lucky Number = 9
```

**Example 2 — Combined is a master number:** 02/04/2000

```text
First digit of month (2) = 2
Last non-zero digit of year (2000) = 2
Combined: 22  →  master number → Lucky Number = 22
```

**Example 3 — Year reduction used:** 11/19/1989

```text
First digit of month (11) = 1
Last non-zero digit of year (1989) = 9
Combined: 19  →  > 9, not master → go to Step 2
Birth day = 19  →  > 9, not master → go to Step 3
Year digits: 1 + 9 + 8 + 9 = 27  →  2 + 7 = 9 → Lucky Number = 9
```

**Example 4 — Year reduction stops at 28:** 12/19/1999

```text
First digit of month (12) = 1
Last non-zero digit of year (1999) = 9
Combined: 19  →  > 9, not master → go to Step 2
Birth day = 19  →  > 9, not master → go to Step 3
Year digits: 1 + 9 + 9 + 9 = 28  →  master number, stop → Lucky Number = 28
```
...
A Personal Year number indicates the numerological theme of a person's current year cycle.

### Personal Year Formula

1. Determine the **year of the person's last birthday**:
   - If today's date is on or after the birth month/day this calendar year → use the **current year**
   - If today's date is before the birth month/day this calendar year → use **last year**

2. **Sum** the following values:
   - Birth month (raw value — month 11 stays as 11, not reduced to 2)
   - Birth day (raw value)
   - Individual digits of the personal year (e.g. 2025 → 2+0+2+5 = 9)

3. **Reduce** the raw total to a single digit or master number (11, 22, 33), stopping at any master number encountered. If reduction produces 2, treat as 11.

4. **Display format**: `finalNumber/rawTotal` (e.g. `11/29`). If both are equal, display the number only.

### Example

Birthday: November 9 (11/09). Today: any date on or after Nov 9, 2025.

```text
Personal Year = 11 + 9 + (2+0+2+5)
             = 11 + 9 + 9
             = 29          <- raw/intermediary
  2 + 9 = 11               <- master number, stop here
Display: 11/29
```
...
| Year | Theme |
| ---- | ----- |
| 1 | New beginnings, independence, initiative |
| 2 | Patience, partnership, sensitivity |
| 3 | Creativity, communication, social expansion |
| 4 | Work, structure, foundation building |
| 5 | Change, freedom, movement |
| 6 | Responsibility, home, relationships |
| 7 | Reflection, study, inner development |
| 8 | Power, ambition, financial movement |
| 9 | Completion, release, endings |
| 11 | Vision, influence, emotionality (master year) |
| 22 | Large-scale building, legacy (master year) |
| 33 | Service, teaching, purpose (master year) |
...
Any birth date where the raw digit sum (using standard master-number rules for month and day) equals exactly **13** produces LP 13/4. Because most year digit sums for modern years are already 10+, LP 13/4 dates are relatively rare and tend to occur in years with very low digit sums (e.g. 2000 → sum 2, 2010 → sum 3).

#### Example — 11/09/2002 (not 13/4)

```text
Month 11 (master, kept) = 11
Day 9 (single digit)    = 9
Year 2002 → 2+0+0+2     = 4
Total: 11+9+4 = 24 → LP 6/24
```

#### Example — 01/01/2002 (not 13/4)

```text
1 + 1 + (2+0+0+2) = 1 + 1 + 4 = 6 → LP 6
```

#### Example — 05/02/2000 (not 13/4)

```text
5 + 2 + (2+0+0+0) = 5 + 2 + 2 = 9 → LP 9
```

#### Example — 09/02/2000 (LP 13/4 ✓)

```text
9 + 2 + (2+0+0+0) = 9 + 2 + 2 = 13 → LP 13/4
```
...
Letterology assigns a numerical value to each letter of a word or name and derives several numerical outputs from those values.

### Letter Values

| Case | Range | Values |
| ---- | ----- | ------ |
| Lowercase | a–z | a=1, b=2, c=3 … z=26 |
| Uppercase | A–Z | A=27, B=28, C=29 … Z=52 |

By default all input letters are treated as lowercase (1–26). When **case sensitive** mode is enabled, each letter retains its case: lowercase letters use values 1–26, uppercase letters use values 27–52.

### Reduction Rules

Reduce a number by summing its digits repeatedly until the result is a single digit (1–9) or a **master number (11, 22, 33)**. Master numbers are never reduced further.

```text
reduce(29) = 2+9 = 11 → stop (master)
reduce(20) = 2+0 = 2  → stop (single digit)
reduce(15) = 1+5 = 6  → stop (single digit)
```
...
1. For each letter in the word, take its positional value and **reduce it** to a single digit or master number.
2. Sum all the per-letter reduced values.
3. Reduce the sum (stopping at master numbers).

```text
Example: "hl" (lowercase)
h = 8  →  reduced: 8
l = 12  →  reduced: 1+2 = 3
Sum = 8+3 = 11  →  master number, stop
Steps (Reduced) = 11
```

#### Steps — Non-Reduced

1. For each letter, use its **raw positional value** (no per-letter reduction).
...
| Value | Definition |
| ----- | ---------- |
| **Total Reduced** | Reduce the sum of per-letter reduced values (same as Steps Reduced result) |
| **First Letter** | The reduced value of the first letter (raw value shown in parentheses) |
| **First Vowel** | The reduced value of the first vowel in the word (raw value shown in parentheses); vowels are a, e, i, o, u |
| **Vowel Sum** | Sum the **raw** values of all vowels, then reduce (stopping at master numbers) |

#### Summary

| Label | Definition |
| ----- | ---------- |
| **Reduced** | Steps (Reduced) result — sum of per-letter-reduced values, reduced to single digit or master |
| **Non-Reduced** | Steps (Non-Reduced) result — sum of raw values, reduced to single digit or master |
| **Final** | Equal to the Reduced result — the authoritative single number for the word, always respecting master number stops |

### Letterology Example

Word: **"hello"** (lowercase, case insensitive)
...
**Steps (Reduced):** 8+5+3+3+6 = 25 → 2+5 = **7**

**Steps (Non-Reduced):** 8+5+12+12+15 = 52 → 5+2 = **7**

**Additional Values:**

- Total Reduced: 7
- First Letter: 8 (raw 8)
- First Vowel: 5 (raw 5) — "e" is the first vowel
- Vowel Sum: e=5, o=15 → 5+15=20 → 2+0 = **2** (raw values used)

**Summary:** Reduced = 7, Non-Reduced = 7, Final = **7**
