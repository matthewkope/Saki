...
export default function CelebrityPage() {
    ...
    async function toggleSave() {
        if (!session || !celebrity) return;
        if (saved) {
            await supabase.from('saved_items').delete()
                .eq('user_id', session.user.id).eq('item_type', 'celebrity').eq('item_id', id);
            setSaved(false);
        } else {
            await supabase.from('saved_items').insert({
                user_id: session.user.id, item_type: 'celebrity',
                item_id: Number(id), item_name: celebrity.name, item_image_url: celebrity.image_url,
            });
            setSaved(true);
        }
    }
    ...
    const compatResult = (profile && celebrity.month && celebrity.day && celebrity.year)
        ? calcScore(
            { month: profile.birth_month, day: profile.birth_day, year: profile.birth_year },
            { month: celebrity.month, day: celebrity.day, year: celebrity.year }
          )
        : null;
    ...
    const baseBubbles = [
        { label: 'Life Path', val: `LP ${celebrity.life_path}`, color: 'var(--accent,#c4a0ff)', bg: 'rgba(196,160,255,0.08)' },
        { label: 'Eastern',   val: celebrity.eastern_zodiac,    color: 'var(--gold,#f0d080)',   bg: 'rgba(240,208,128,0.08)' },
        { label: 'Western',   val: celebrity.western_zodiac,    color: '#a78bfa',               bg: 'rgba(167,139,250,0.08)' },
    ];
    ...
    return (
        <div style={{ padding: '40px 20px', maxWidth: '900px', margin: '0 auto', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column', gap: '40px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button onClick={() => router.push('/celebrities')} style={{ background: 'transparent', color: '#666', border: '1px solid #333', padding: '6px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '0.9rem' }}>
                    ← All Celebrities
                </button>
                {session && (
            ...
            <div style={{ display: 'flex', gap: '32px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: '220px' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                        ...
                        {compatResult && relStyle && profile && celebrity.month && celebrity.day && celebrity.year && (
                            <div
                                onClick={() => router.push(`/compatibility?p1m=${profile.birth_month}&p1d=${profile.birth_day}&p1y=${profile.birth_year}&p2m=${celebrity.month}&p2d=${celebrity.day}&p2y=${celebrity.year}`)}
            ...
            {celebrity.description && (
                <div style={{ background: 'var(--surface, #12121a)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '28px 32px' }}>
                    ...
                    <p style={{ color: '#ccc', fontSize: '0.95rem', lineHeight: 1.75, margin: 0 }}>{celebrity.description}</p>
                </div>
            )}
        </div>
    );
